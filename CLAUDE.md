...
**Dispersión de puntería (v2.9.0)**: con `assists.aimScatter`, cada bola disparada tira su propio dado en `applyAimScatter`. Si sale, se desvía un ángulo uniforme dentro de ±`maxDegrees`, recortado a `[AIM_ANGLE_MIN, AIM_ANGLE_MAX]` (los mismos topes que usa el apuntado manual en `input.js`). Cada ball behavior puede declarar su propio `aimScatter: { chance, maxDegrees }`; los que no, usan `DEFAULT_AIM_SCATTER` de `config.js`. Valores actuales: normal 20%/±4°, fuego 25%/±5°, divisora 30%/±6°, fuerza 30%/±6°, bomba 35%/±7°.
